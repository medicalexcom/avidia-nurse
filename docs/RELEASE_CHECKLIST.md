...
- [~] Author seed content: target 100–300 reviewed questions across Med-Surg/Fundamentals/Pharm and ~10 cases (RN validation before canonical promotion). Editorial/clinical work — not something to generate unreviewed given this is nursing-education content patients' care decisions could trace back to. **2026-08-25: the review tool itself turned out to already exist and already work** (`content-review` function + `app/(app)/review.tsx`, founder account already `role = 'reviewer'` — open it via Profile → "Open review queue"). What was actually missing: the generation pipeline skipped it — clean-passing AI questions went straight to `active` (live, unreviewed) instead of waiting in the queue. Fixed in `packages/assessment/src/validate.ts` (code) + `supabase/migrations/0023_generated_status_review_gate.sql` (staged, needs founder to run in the SQL editor — see `docs/KNOWN_LIMITATIONS.md`). Once that migration runs, new questions wait for review by default; the 116 already-`active` questions are untouched by it and were never reviewed by anyone. Remaining: actually reviewing content through the tool is still real editorial work, and 0023 needs to be run before it starts mattering for new generations.
