...
2. In the SQL Editor, run each file in `migrations/` in filename order
   ...
   `0005_source_chunks_and_retrieval.sql`).
   ...
   `search_course_chunks` functions.
...
## Running the worker (M4 extraction + M5 indexing)

The background worker extracts uploaded materials into `document_sections`
and then indexes ready documents into `source_chunks` (semantic chunks +
embeddings) so retrieval works. It needs the project URL, the
**service-role key** (Project Settings → API), and an embedding provider —
all server-side environment variables — these are secrets: never in `.env`
...
export OPENAI_API_KEY=<openai api key>     # embeddings (text-embedding-3-small)
...
denied to anonymous callers; course deletion cascades to chunks), then
